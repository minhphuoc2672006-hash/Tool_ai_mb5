...
def repair_state(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    d.setdefault("cooldown_active", False)
    d.setdefault("cooldown_reason", "")
    d.setdefault("last_relearn_note", "")
    d.setdefault("last_resume_note", "")
    d.setdefault("last_relearn_snapshot", "")
    d.setdefault("last_relearn_total", 0)
...
def analyze_state_from_labels(state: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    # Nếu đang học lại nhưng đã có kết quả đúng -> mở lại
    resumed = False
    if state.get("cooldown_active") and int(state.get("current_wrong_streak", 0)) == 0:
        resumed = clear_loss_cooldown(state)

    # Nếu vừa chạm chuỗi thua 5 -> reset logic/model và khóa phân tích
    relearned = False
...
def build_bridge_chart_image(
    ...
    if len(ys) >= 5:
        ...
        ax.plot(xs, ma, linewidth=1.5, alpha=0.8, linestyle="--", color="#7db7ff", zorder=4)
    ...
    if len(ys) >= 12:
        ...
        for i in range(len(ys)):
            ...
            seg = ys[start:i + 1]
            ma.append(sum(seg) / len(seg))
        ax.plot(xs, ma, linewidth=1.8, alpha=0.65, linestyle=":", color="#ffb347", zorder=4)
...
def build_stats_message(report: Dict[str, Any], state: Dict[str, Any], adv: Dict[str, Any]) -> str:
    ...
    cooldown_reason = state.get("cooldown_reason") or "-"
    relearn_note = state.get("last_relearn_note") or state.get("last_resume_note") or "-"
    ...
    return (
        ...
        f"║ Lý do HL : {cooldown_reason}\n"
        f"║ Ghi chú  : {relearn_note}\n"
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    async with STATE_LOCK:
        ...
        if entries:
            ...
            # Nếu đang học lại mà đã có kết quả đúng thì mở lại
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with STATE_LOCK:
        ...
        # Nếu có thể thoát khỏi học lại thì mở lại
        clear_loss_cooldown(state)
