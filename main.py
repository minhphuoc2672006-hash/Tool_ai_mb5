...
def ensure_robot_animation() -> str:
    ...
    base_path = ROBOT_IMAGE_PATH
    if not os.path.exists(base_path):
        base_path = ensure_robot_asset()
    ...
    try:
        ...
        for i in range(16):
            phase = i / 16.0
            scale = 1.0 + 0.03 * math.sin(2 * math.pi * phase)
            new_w = max(1, int(w * scale))
            new_h = max(1, int(h * scale))

            fr = src.resize((new_w, new_h), Image.LANCZOS)
            canvas = Image.new("RGB", (w, h), (245, 245, 245))

            x = (w - new_w) // 2
            y = (h - new_h) // 2 + int(8 * math.sin(2 * math.pi * phase))
            canvas.paste(fr, (x, y))

            bright = 0.95 + 0.12 * (0.5 + 0.5 * math.sin(2 * math.pi * phase))
            canvas = ImageEnhance.Brightness(canvas).enhance(bright)

            glow = canvas.filter(ImageFilter.GaussianBlur(radius=2))
            blended = Image.blend(canvas, glow, alpha=0.15)
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Luồng hoạt động: cập nhật thống kê → đọc biểu đồ → robot phân tích → chốt cuối."
    )
