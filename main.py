...
def parse_input(text: str) -> List[int]:
    ...
    Supported forms:
    - Single total: "12"
    - Three dice: "1 3 5", "1-3-5", "135"
    - Multiple rolls in one message: "1 3 5 / 2 4 6"

    If the numbers look like dice faces (1..6), the function groups them
    in triples and returns the sum of each triple.
    """
    text = (text or "").strip()
    if not text:
        return []
    ...
    # Packed three-dice input like "135"
    if len(tokens) == 1:
        tok = tokens[0]
    ...
    # If all tokens are dice faces 1..6, treat them as grouped rolls.
    ...
    # Fallback: every number is treated as a standalone result/total.
    out: List[int] = []
    for x in tokens:
        try:
            n = int(x)
