...
# CHART
# =========================
def build_bridge_chart_image(labels: List[str], limit: int = 30) -> Optional[BytesIO]:
    ...
    ax.set_ylabel("Cầu")
    ax.set_title(f"Biểu đồ cầu - {len(tail)} mẫu gần nhất")
    ...
    ax.text(xs[-1], ys[-1] + 0.05, tail[-1], ha="center", va="bottom", fontsize=9)
...
def build_chart_caption(report: Dict[str, Any], title: str) -> str:
    return (
        f"{title}\n"
...
async def send_bridge_chart(
    ...
    chart = build_bridge_chart_image(labels)
    if chart is None:
    ...
    try:
        await update.message.reply_photo(
            photo=InputFile(chart, filename="bridge_chart.png"),
            caption=build_chart_caption(report, title),
        )
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    await send_bridge_chart(update, full_labels, report, "📈 BIỂU ĐỒ CẦU - CẬP NHẬT MỚI")
    ...
    await send_bridge_chart(update, full_labels, report, "📈 BIỂU ĐỒ CẦU - PHÂN TÍCH")
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_bridge_chart(update, labels, report, "📈 BIỂU ĐỒ CẦU - THỐNG KÊ MỚI NHẤT")
    await update.message.reply_text(build_stats_message(report, state, adv))
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(build_stage_message(1))
    await send_bridge_chart(update, labels, report, "📈 BIỂU ĐỒ CẦU - DÙNG CHO PHÂN TÍCH")
    await update.message.reply_text(build_stats_message(report, state, adv))
