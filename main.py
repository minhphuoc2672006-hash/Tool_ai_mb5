...
def meta_decision(
    ...
    trend_gap = last_5_high - prev_5_high
    ...
    for name, pred in predictions.items():
        ...
        if abs(trend_gap) >= 0.25:
            if trend_gap > 0 and label == HIGH_LABEL:
                score *= 1.08
            elif trend_gap < 0 and label == LOW_LABEL:
                score *= 1.08
...
# CHART
# =========================
def build_chart_summary(report: Dict[str, Any], adv: Dict[str, Any]) -> str:
    structure = report.get("structure", "-")
    detail = report.get("detail", "-")
    total = int(report.get("total", 0))
    low = int(report.get("low", 0))
    high = int(report.get("high", 0))
    dom_gap = float(report.get("dominance_gap", 0.0))
    ...
    return (
        ...
        f"Cấu trúc: {structure} | Lệch: {dom_gap:.2f}\n"
        f"Chi tiết: {detail}\n"
...
def build_bridge_chart_image(
    ...
    tail = labels[-limit:] if len(labels) > limit else labels[:]
    if not tail:
        return None
    ...
    ax.set_title("BIỂU ĐỒ CẦU PHÂN TÍCH")
    ...
    stats_text = build_chart_summary(report, adv)
    ax.text(
        0.02,
        0.98,
        stats_text,
...
async def send_bridge_chart(
    ...
    chart = build_bridge_chart_image(labels, report, adv)
    if chart is None:
    ...
    try:
        ...
        await update.message.reply_photo(
            photo=chart,
            caption=f"{title}\n{build_chart_summary(report, adv)}",
        )
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    await send_bridge_chart(update, full_labels, report, adv, "📈 BIỂU ĐỒ CẦU - CẬP NHẬT MỚI")
    ...
    await send_bridge_chart(update, full_labels, report, adv, "📈 BIỂU ĐỒ CẦU - PHÂN TÍCH")
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_bridge_chart(update, labels, report, adv, "📈 BIỂU ĐỒ CẦU - THỐNG KÊ MỚI NHẤT")
    await update.message.reply_text(build_stats_message(report, state, adv))
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(build_stage_message(1))
    await send_bridge_chart(update, labels, report, adv, "📈 BIỂU ĐỒ CẦU - DÙNG CHO PHÂN TÍCH")
    await update.message.reply_text(build_stats_message(report, state, adv))
