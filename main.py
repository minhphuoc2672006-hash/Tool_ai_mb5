...
# ===== AI NGHỊCH ĐẢO THÔNG MINH =====
def detect_inverse_hand(history):
    tx = ["Tài" if sum(x)>=11 else "Xỉu" for x in history]
    if len(tx) < 5:
        return False
...
def ai_predict(user):
    ...
    if len(history) < 5:
        # Chưa đủ 5 lượt → không phân tích
        return None, None
    ...
    if detect_inverse_hand(history):
        pred = "Xỉu" if c.most_common(1)[0][0]=="Tài" else "Tài"
    else:
        pred = c.most_common(1)[0][0]

    conf = random.uniform(60, 75)
    return pred, conf
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== XỬ LÝ KẾT QUẢ =====
    if user["last_pred"] is not None and user["last_bet"] > 0:
