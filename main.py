...
def repair_state(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    d.setdefault("last_detected_pattern", "")
    d.setdefault("last_detected_hint", None)
...
def detect_early_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    # giữ lại như một helper để hỗ trợ các mẫu rất sớm trong pattern,
    # nhưng không dùng làm model riêng.
    out: List[Dict[str, Any]] = []
    if len(labels) < 4:
        return out
    ...
    if len(tail4) == 4 and tail4[0] != tail4[1] and tail4[0] == tail4[2] and tail4[1] == tail4[3]:
        out.append({"name": "1-1", "detail": "Luân phiên rất sớm", "score": 92, "hint": tail4[0]})

    if len(tail6) == 6:
        changes = sum(1 for i in range(1, 6) if tail6[i] != tail6[i - 1])
        if changes >= 4:
            out.append({"name": "XEN KẼ SỚM", "detail": "Đổi nhịp nhanh trong 6 mẫu gần nhất", "score": 89, "hint": opposite_label(tail6[-1])})

    if len(tail5) == 5 and len(set(tail5[-4:])) == 1:
        out.append({"name": "BỆT SỚM", "detail": f"{tail5[-1]} đang lặp sớm", "score": 90, "hint": tail5[-1]})
...
def predict_structure(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        strong_same = {
            ...
            "XEN KẼ SỚM", "XEN KẼ", "XEN KẼ SÂU", "1-1", "2-2",
        }
...
def build_chart_summary(report: Dict[str, Any], adv: Dict[str, Any], state: Optional[Dict[str, Any]] = None) -> str:
    ...
    return (
        ...
        f"Chốt: {state.get('last_gate_status', 'CHỜ') if state else 'CHỜ'}"
    )


def _point_style(idx: int, y: int, total: int) -> Tuple[str, str, str]:
    # facecolor, edgecolor, textcolor
    if idx == total - 1:
        return ("#22d3ee", "#e5e7eb", "#0f172a")
    if y == 1:
        return ("#f6c445", "#7c5b00", "#111111")
    return ("#f4f4f5", "#71717a", "#111111")
...
def build_bridge_chart_image(
    ...
    fig_w = max(13.0, min(38.0, 0.24 * len(tail) + 10.0))
    fig_h = 6.9 if len(tail) < 120 else 7.5
    fig, ax = plt.subplots(figsize=(fig_w, fig_h), dpi=180)
    fig.patch.set_facecolor("#07121f")
    ax.set_facecolor("#07121f")

    for spine in ax.spines.values():
        spine.set_color("#1f2937")

    ax.grid(True, which="major", axis="both", color="#334155", alpha=0.45, linewidth=0.9)
    ax.set_axisbelow(True)

    # vùng nền giống kiểu biểu đồ trong ảnh mẫu
    ax.axhspan(-0.1, 0.5, color="#0b1724", alpha=0.35)
    ax.axhspan(0.5, 1.1, color="#0d2435", alpha=0.20)
    ax.axhline(0.5, linewidth=1.2, linestyle="--", color="#94a3b8", alpha=0.55)

    ax.plot(xs, ys, linewidth=3.0, color="#facc15", zorder=2)

    # đường trung bình động để nhìn cầu rõ hơn
    if len(ys) >= 5:
        w = 5
        ma = []
        ...
        ax.plot(xs, ma, linewidth=1.5, linestyle="--", color="#38bdf8", alpha=0.65, zorder=1)
    ...
    if len(ys) >= 12:
        ...
        for i in range(len(ys)):
            ...
            seg = ys[start:i + 1]
            ma.append(sum(seg) / len(seg))
        ax.plot(xs, ma, linewidth=1.7, linestyle=":", color="#a78bfa", alpha=0.60, zorder=1)

    # điểm + nhãn hình tròn
    for i, (x, y, val) in enumerate(zip(xs, ys, tail)):
        face, edge, txt = _point_style(i, y, len(xs))
        ax.scatter([x], [y], s=380, color=face, edgecolors=edge, linewidths=2.0, zorder=4)
        ax.text(
            x,
            y,
            str(val),
    ...
    # nhấn mạnh điểm cuối
    ax.scatter([xs[-1]], [ys[-1]], s=540, facecolors="none", edgecolors="#eab308", linewidths=2.2, zorder=3)

    ax.set_ylim(-0.18, 1.18)
    ax.set_yticks([0, 1])
    ax.set_yticklabels([LOW_LABEL, HIGH_LABEL], color="#e5e7eb", fontsize=11)
    ax.tick_params(axis="x", colors="#cbd5e1", labelsize=9)
    ax.tick_params(axis="y", colors="#e5e7eb", labelsize=11)

    # Hiển thị x theo kiểu biểu đồ mẫu: ít chữ, nhiều ô lưới
    if len(xs) <= 20:
        ax.set_xticks(xs)
        ax.set_xticklabels([str(i) for i in xs], color="#cbd5e1")
    else:
        step = max(1, len(xs) // 12)
        ticks = xs[::step]
        ax.set_xticks(ticks)
        ax.set_xticklabels([str(i) for i in ticks], color="#cbd5e1")

    ax.set_xlabel("Mẫu gần nhất", color="#e5e7eb")
    ax.set_ylabel("Trạng thái", color="#e5e7eb")
    ax.set_title("BIỂU ĐỒ CẦU PHÂN TÍCH - TOÀN BỘ LỊCH SỬ", color="#f8fafc", fontsize=13, fontweight="bold")
    ...
    ax.text(
        ...
        fontsize=9.3,
        color="#0f172a",
        bbox=dict(boxstyle="round,pad=0.55", facecolor="#e5e7eb", alpha=0.90, edgecolor="#94a3b8"),
        zorder=10,
    )
    ...
    ax.text(
        ...
        fontsize=9.3,
        color="#0f172a",
        bbox=dict(boxstyle="round,pad=0.4", facecolor="#e5e7eb", alpha=0.90, edgecolor="#94a3b8"),
        zorder=10,
    )
...
def build_stage_message(step: int) -> str:
    ...
    if step == 2:
        return "🔍 Bước 2: Đã phân tích cầu."
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Bot chỉ phân tích khi có cầu rõ hoặc cầu sớm đủ mạnh.\n"
        f"Luồng hoạt động: cập nhật thống kê → biểu đồ cầu → phân tích cầu."
    )
