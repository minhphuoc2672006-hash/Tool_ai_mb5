...
class SuperAIManager:
    ...
    def monitor_user(self, user):
        ...
        last_total = classify_total(sum(user["history"][-1]))
        if user["last_pred"] is not None:
            if user["last_pred"] == last_total:
                self.global_stats["wins"] += 1
            else:
                self.global_stats["losses"] += 1

        if user.get("lose",0) >= 1:
            old_strategy = user["strategy"]
            user["strategy"] = pick_strategy()
            logging.info(f"⚡ User thua liên tục, đổi chiến thuật: {old_strategy} → {user['strategy']}")

        pred, conf = ai_predict(user)
        if pred:
            total_played = user["win"] + user["lose"]
            if total_played > 5 and (user["win"]/total_played)<0.5:
                conf = min(0.75, conf + 0.1)
            user["last_pred"] = pred
            user["last_conf"] = conf

        patterns = analyze_patterns(user["history"])
        if patterns.get("phức tạp",0)>=1 and random.random()>0.5:
            old_strategy = user["strategy"]
            user["strategy"] = pick_strategy()
            logging.info(f"⚡ Phát hiện cầu phức tạp, đổi chiến thuật: {old_strategy} → {user['strategy']}")

    def summary(self):
        total = self.global_stats["total_games"]
        wins = self.global_stats["wins"]
        losses = self.global_stats["losses"]
        winrate = (wins / total *100) if total>0 else 0
        return f"📊 Global Summary: Total {total}, Wins {wins}, Losses {losses}, Winrate {winrate:.1f}%"

super_ai_manager = SuperAIManager()
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Super AI quản lý tổng thể
    super_ai_manager.monitor_user(user)
    await update.message.reply_text(super_ai_manager.summary())
