...
def ensure_robot_asset() -> str:
    ...
    # Tạo ảnh robot fallback nếu chưa có sẵn.
    try:
...
def ensure_robot_animation() -> str:
    ...
    try:
        ...
        # Tạo animation nhẹ: nhấp nháy mắt, rung nhè nhẹ, halo dưới chân, hạt sáng quay vòng.
        for i in range(24):
            phase = i / 24.0

            frame = src.copy()
            # bobbing + zoom rất nhẹ
            ...
            # Tọa độ xấp xỉ cho ảnh robot mà bạn gửi
            ...
            # Eye blink overlay
            # Dùng overlay đơn giản để tạo cảm giác chớp mắt
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    if result.get("relearned", False):
        await update.message.reply_text("🧠 BOT ĐANG TỰ HỌC LẠI SAU CHUỖI THUA.")
    if result.get("resumed", False):
        await update.message.reply_text("✅ BOT ĐÃ MỞ LẠI CẦU.")

    if not result.get("allowed", False):
        await update.message.reply_text(
            f"⏸ BOT TẠM DỪNG\n"
            f"Lý do: {result.get('reason', 'Cầu chưa rõ')}\n"
            f"Cấu trúc: {report.get('structure', '-')}\n"
            f"Chi tiết: {report.get('detail', '-')}"
        )
        return
