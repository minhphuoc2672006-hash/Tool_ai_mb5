...
MAX_KEEP_HISTORY = int(os.getenv("MAX_KEEP_HISTORY", "0"))  # 0 = giữ toàn bộ
...
HISTORY_ANALYSIS_LIMIT = int(os.getenv("HISTORY_ANALYSIS_LIMIT", "0"))  # 0 = toàn bộ lịch sử
...
def repair_state(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    d.setdefault("current_correct_streak", 0)
    d.setdefault("current_wrong_streak", 0)
    d.setdefault("max_correct_streak", 0)
    d.setdefault("max_wrong_streak", 0)
...
def detect_all_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    patterns.extend(detect_explicit_pair_patterns(labels))
    ...
    early_patterns = detect_early_patterns(labels)
    if early_patterns:
        patterns.extend(early_patterns)
...
def extract_chart_features(labels: List[str]) -> Dict[str, Any]:
    ...
    trend = slope * 100.0
    if trend > 0.08:
        trend_label = f"NGHIÊNG {HIGH_LABEL}"
    elif trend < -0.08:
        trend_label = f"NGHIÊNG {LOW_LABEL}"
    else:
        trend_label = "TRUNG TÍNH"
    ...
    return {
        ...
        "max_streak": max_streak,
        "last_value": ys[-1],
        "last_5_high": last_5_high,
        "prev_5_high": prev_5_high,
    }



def detect_early_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    if len(labels) < 4:
        return out

    tail4 = labels[-4:]
    tail5 = labels[-5:] if len(labels) >= 5 else labels[:]
    tail6 = labels[-6:] if len(labels) >= 6 else labels[:]
    tail8 = labels[-8:] if len(labels) >= 8 else labels[:]
    tail10 = labels[-10:] if len(labels) >= 10 else labels[:]

    if len(tail4) == 4 and tail4[0] != tail4[1] and tail4[0] == tail4[2] and tail4[1] == tail4[3]:
        out.append({
            "name": "1-1",
            "detail": "Luân phiên rất sớm",
            "score": 92,
            "hint": tail4[0],
        })

    if len(tail6) == 6:
        changes = sum(1 for i in range(1, 6) if tail6[i] != tail6[i - 1])
        if changes >= 4:
            out.append({
                "name": "XEN KẼ SỚM",
                "detail": "Đổi nhịp nhanh trong 6 mẫu gần nhất",
                "score": 89,
                "hint": opposite_label(tail6[-1]),
            })

    if len(tail5) == 5 and len(set(tail5[-4:])) == 1:
        out.append({
            "name": "BỆT SỚM",
            "detail": f"{tail5[-1]} đang lặp sớm",
            "score": 90,
            "hint": tail5[-1],
        })

    if len(tail8) >= 8:
        rle = run_length_encode(tail8)
        if len(rle) >= 3:
            a, la = rle[-3]
            b, lb = rle[-2]
            c, lc = rle[-1]
    ...
    if len(tail10) >= 10:
        c = Counter(tail10)
        if abs(c[HIGH_LABEL] - c[LOW_LABEL]) == 0:
            out.append({
                "name": "CÂN BẰNG SỚM",
                "detail": "Đuôi ngắn đang cân bằng",
                "score": 78,
                "hint": None,
            })

    return sorted(out, key=lambda x: x.get("score", 0), reverse=True)[:6]


def predict_early(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        if name in {"BỆT SỚM", "XEN KẼ SỚM", "1-1", "XEN KẼ", "BỆT", "NHỊP SỚM"} and hint in (LOW_LABEL, HIGH_LABEL):
            return {
                "label": hint,
                "confidence": min(95, int(p.get("score", 60)) + 1),
                "source": f"early:{name}",
            }

    if len(labels) < 4:
        return {"label": labels[-1] if labels else LOW_LABEL, "confidence": 50, "source": "early"}

    tail = labels[-4:]
    if tail[0] == tail[2] and tail[1] == tail[3] and tail[0] != tail[1]:
        return {"label": tail[0], "confidence": 82, "source": "early:1-1"}

    if len(labels) >= 5 and labels[-1] == labels[-2] == labels[-3] == labels[-4]:
        return {"label": labels[-1], "confidence": 84, "source": "early:bett"}

    if len(labels) >= 6:
        tail6 = labels[-6:]
        changes = sum(1 for i in range(1, 6) if tail6[i] != tail6[i - 1])
        if changes >= 4:
            return {"label": opposite_label(tail6[-1]), "confidence": 80, "source": "early:alt"}
...
def predict_structure(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        strong_same = {
            ...
            "XEN KẼ SỚM", "XEN KẼ", "XEN KẼ SÂU", "1-1", "2-2", "NHỊP SỚM",
        }
...
def meta_decision(
    ...
    strong_patterns = {
        ...
        "1-1", "2-2", "NHỊP SỚM", "LẶP KHỐI 1", "LẶP MẪU 2",
    ...
    for name, pred in predictions.items():
        ...
        if report.get("structure") in strong_patterns:
            if name in {"pattern", "structure", "early"}:
                score *= 1.08
...
def analyze_state_from_labels(state: Dict[str, Any], labels: List[str]) -> Dict[str, Any]:
    ...
    predictions = {
        ...
        "early": predict_early(labels, report),
    }
...
def build_chart_summary(report: Dict[str, Any], adv: Dict[str, Any], state: Optional[Dict[str, Any]] = None) -> str:
    ...
    entropy = float(report.get("entropy", 0.0))
    max_streak = int(adv.get("max_streak", 0))
    r10 = float(adv.get("r10_high", 0.5)) * 100
    r20 = float(adv.get("r20_high", 0.5)) * 100
    momentum = float(adv.get("momentum", 0.0))
    reversal = float(adv.get("reversal", 0.0))
    trend_label = adv.get("trend_label", "TRUNG TÍNH")
    trend = float(adv.get("trend", 0.0))
    smoothness = float(adv.get("smoothness", 0.0))

    pred_total = int((state or {}).get("prediction_total", 0))
    pred_hits = int((state or {}).get("prediction_hits", 0))
    pred_miss = int((state or {}).get("prediction_misses", 0))
    pred_acc = safe_div(pred_hits * 100.0, pred_total)
    max_win = int((state or {}).get("max_correct_streak", 0))
    max_lose = int((state or {}).get("max_wrong_streak", 0))

    return (
        f"Tổng: {total} | {LOW_LABEL}: {low} | {HIGH_LABEL}: {high}\n"
        f"Cấu trúc: {structure}\n"
        f"Chi tiết: {detail}\n"
        f"Bệt max: {max_streak} | 10 gần: {r10:.1f}% {HIGH_LABEL} | 20 gần: {r20:.1f}% {HIGH_LABEL}\n"
        f"Momentum: {momentum:.2f} | Trend: {trend_label} ({trend:.2f})\n"
        f"Mượt: {smoothness:.2f} | Đảo chiều: {reversal:.1f}%\n"
        f"Entropy: {entropy:.2f} | Volatility: {vol:.2f}\n"
        f"Chốt: {pred_hits}/{pred_miss} đúng/sai | Tổng chốt: {pred_total} | Acc: {pred_acc:.1f}%\n"
        f"Thắng tối đa: {max_win} | Thua tối đa: {max_lose}"
    )
...
def build_bridge_chart_image(
    ...
    fig_w = max(12.0, min(36.0, 0.22 * len(tail) + 10.0))
    fig_h = 6.8 if len(tail) < 120 else 7.4
    fig, ax = plt.subplots(figsize=(fig_w, fig_h), dpi=170)

    ax.axhspan(0.0, 0.5, alpha=0.08)
    ax.axhspan(0.5, 1.0, alpha=0.08)
    ax.axhline(0.5, linewidth=1.1, linestyle="--", alpha=0.55)

    ax.step(xs, ys, where="mid", linewidth=2)
    ax.scatter(xs, ys, s=28, zorder=3)

    ax.scatter([xs[-1]], [ys[-1]], s=90, zorder=4)
    ax.annotate(
        tail[-1],
        (xs[-1], ys[-1]),
        textcoords="offset points",
        xytext=(0, 10 if ys[-1] == 1 else -18),
        ha="center",
        fontsize=10,
        fontweight="bold",
    )

    # Đường trung bình động để nhìn xu hướng dài hạn rõ hơn
    if len(ys) >= 5:
        w = 5
        ma = []
        ...
        ax.plot(xs, ma, linewidth=1.4, alpha=0.85, linestyle="--")
    ...
    if len(ys) >= 12:
        ...
        for i in range(len(ys)):
            ...
            seg = ys[start:i + 1]
            ma.append(sum(seg) / len(seg))
        ax.plot(xs, ma, linewidth=1.8, alpha=0.75, linestyle=":")

    ax.set_ylim(-0.15, 1.15)
    ax.set_yticks([0, 1])
    ax.set_yticklabels([LOW_LABEL, HIGH_LABEL])
    ax.set_xlabel("Mẫu gần nhất")
    ax.set_ylabel("Trạng thái")
    ax.set_title("BIỂU ĐỒ CẦU PHÂN TÍCH - TOÀN BỘ LỊCH SỬ")
    ...
    stats_text = build_chart_summary(report, adv, state)
    ax.text(
        0.02,
        0.98,
        stats_text,
        ...
        ha="left",
        fontsize=9,
        bbox=dict(boxstyle="round,pad=0.5", facecolor="white", alpha=0.82, edgecolor="gray"),
    )
    ...
    ax.text(
        ...
        ha="right",
        fontsize=9,
        bbox=dict(boxstyle="round,pad=0.4", facecolor="white", alpha=0.72, edgecolor="gray"),
    )
...
def build_stats_message(report: Dict[str, Any], state: Dict[str, Any], adv: Dict[str, Any]) -> str:
    ...
    model_acc_line = (
        ...
        f"E:{int(model_acc.get('early', 50))}%"
    )
...
def build_analysis_message(report: Dict[str, Any], adv: Dict[str, Any], meta: Dict[str, Any], predictions: Dict[str, Dict[str, Any]]) -> str:
    ...
    chart_read = (
        ...
        f"{LOW_LABEL} {report.get('low', 0)} | "
        f"{HIGH_LABEL} {report.get('high', 0)} | "
    ...
    return (
        ...
        f"║ EarlyAI  : {predictions.get('early', {}).get('label', '-')} ({predictions.get('early', {}).get('confidence', 0)}%)\n"
...
async def process_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, nums: Optional[List[int]] = None) -> None:
    ...
    async with STATE_LOCK:
        ...
        if entries:
            # Chỉ dùng kết quả cuối cùng vừa nhập để đối chiếu dự đoán đang chờ.
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Luồng hoạt động: cập nhật thống kê → biểu đồ cầu → phân tích → chốt cuối.\n"
