...
"""
Telegram bot phân tích Tài/Xỉu - bản gộp 1 file, đã lược bỏ phần dư.
...
ANALYSIS_WINDOW_MIN = 25
ANALYSIS_WINDOW_MAX = 25
...
def support_score(labels: List[str], hint: Optional[str], windows: Tuple[int, ...] = (6, 8, 10, 12, 16, 20, 24)) -> int:
...
def infer_tail_next_label(labels: List[str]) -> Tuple[Optional[str], int, str]:
    ...
    tail = labels[-25:] if len(labels) > 25 else labels[:]
    if len(tail) < 4:
...
def new_state() -> Dict[str, Any]:
    return {
        ...
        "max_wrong_streak": 0,
        "model_accuracy": {"pattern": 50, "structure": 50, "chart": 50, "break_ai": 50},
        "last_note": "",
...
def repair_state(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(d.get("model_accuracy"), dict):
        d["model_accuracy"] = {"pattern": 50, "structure": 50, "chart": 50, "break_ai": 50}
    defaults = new_state()
...
def detect_bias(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if gap24 >= 0.25:
        ...
        return {"name": "XU HƯỚNG", "detail": f"25 mẫu gần đây nghiêng về {winner}", "score": 80, "hint": winner}
    if gap24 < 0.10:
...
def detect_all_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    for item in (
        ...
        *detect_dynamic_patterns(labels),
        ...
        detect_chaos_pattern(labels),
        detect_lut_pattern(labels),
...
def web_scan_history(labels: List[str]) -> Optional[Dict[str, Any]]:
    tail = labels[-25:] if len(labels) > 25 else labels[:]
    if len(tail) < 6:
        return None

    candidates: List[Dict[str, Any]] = []
    windows = (6, 8, 10, 12, 16, 20, 24, 25)
...
def update_model_accuracy(state: Dict[str, Any], predictions: Dict[str, Dict[str, Any]], actual: str) -> None:
    state.setdefault("model_accuracy", {"pattern": 50, "structure": 50, "chart": 50, "break_ai": 50})
...
def filter_predictions_for_decision(
    ...
    thresholds = {"chart": 50, "pattern": 50, "structure": 50, "break_ai": 50}
...
def meta_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not vote:
        fallback = predictions.get("chart") or next(iter(predictions.values()), None)
        if not fallback:
            return {
                "model": "none",
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str]) -> Dict[str, Any]:
    ...
    web_scan = web_scan_history(labels)
    if web_scan:
    ...
    if not allowed:
        ...
        return {"report": report, "adv": adv, "chart_features": chart_features, "predictions": {}, "meta": {}, "allowed": False, "reason": reason, "window": window, "labels_used": labels}
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Luồng hoạt động: nhập dữ liệu -> quét 25 mẫu gần nhất -> chốt cuối."
    )
...
def build_trace_message(state: Dict[str, Any]) -> str:
    ...
    if isinstance(model_preds, dict) and model_preds:
        model_text = " | ".join(f"{k}:{v.get('label', '-')}({int(v.get('confidence', 0))}%)" for k, v in model_preds.items())
    else:
        model_text = "-"
    return (
