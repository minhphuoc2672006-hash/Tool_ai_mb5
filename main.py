...
"""
Telegram bot phân tích Tài/Xỉu - bản gộp 1 file.
...
Chạy:
    ...
    python tai_xiu_bot_deep_fixed.py
"""
...
logger = logging.getLogger("tai_xiu_bot_deep_fixed")
...
MIN_FINAL_CONFIDENCE = int(os.getenv("MIN_FINAL_CONFIDENCE", "60"))
MAX_FINAL_CONFIDENCE = int(os.getenv("MAX_FINAL_CONFIDENCE", "90"))

ROBOT_ANALYZE_DELAY = float(os.getenv("ROBOT_ANALYZE_DELAY", "0.7"))
...
def clamp_confidence(value: float, low: int = MIN_FINAL_CONFIDENCE, high: int = MAX_FINAL_CONFIDENCE) -> int:
    return max(low, min(high, int(round(value))))

def confidence_jitter(base: float, spread: float = 2.0) -> float:
    """Small random variation so the final confidence feels less rigid."""
    try:
...
def infer_pattern_hint(labels: List[str], pattern: Dict[str, Any]) -> Optional[str]:
    ...
    tail_hint, _, _ = infer_tail_next_label(labels)
...
def finalize_recheck_label(
    ...
    if raw_score >= 84 and (raw_support >= 2 or conflict_count >= 2):
        ...
        meta["confidence"] = clamp_confidence(max(int(meta.get("confidence", 50)), min(96, raw_score + raw_support * 2)))
    ...
    if patterns:
        ...
        if top_name in {"1-1", "2-2", "XEN KẼ", "XEN KẼ SÂU", "BỆT", "BỆT SỚM", "LẶP MẪU 1", "LẶP KHỐI 1", "SÓNG ĐỀU", "SÓNG ĐẢO", "CẦU PHỨC HỢP", "CHU KỲ MỞ RỘNG", "CHUYỂN PHA", "ĐẢO CHIỀU"} and top_score >= 72:
            if raw_support >= 3 and raw_score >= 70:
                meta["final_label"] = raw_hint
                meta["confidence"] = clamp_confidence(max(int(meta.get("confidence", 50)), min(95, raw_score + 3 + raw_support)))
...
def new_state() -> Dict[str, Any]:
    return {
        ...
        "max_wrong_streak": 0,
        "model_accuracy": {"pattern": 50, "markov": 50, "chart": 50, "structure": 50, "break_ai": 50},
        "last_note": "",
...
def detect_explicit_pair_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    if alt_len >= 4:
        ...
        out.append({"name": "1-1", "detail": f"Chuỗi luân phiên {alt_len} nhịp liên tiếp", "score": min(90 + max(0, alt_len - 4) * 2, 96), "hint": next_label})
        if alt_len >= 6:
            out.append({"name": "XEN KẼ SÂU", "detail": f"Luân phiên đều trong {alt_len} mẫu gần nhất", "score": min(94 + max(0, alt_len - 6), 98), "hint": next_label})
...
def detect_false_trap(labels: List[str]) -> Optional[Dict[str, Any]]:
    ...
    for side in (HIGH_LABEL, LOW_LABEL):
        ...
        if tail[-6:-3] == [side, side, side] and tail[-3] == other and tail[-2:] == [side, side]:
            return {"name": "CẦU DỤ", "detail": f"Cụm {side} x3 bị chọc 1 nhịp rồi hồi lại", "score": 89, "hint": None}
    return None

def detect_micro_break(labels: List[str]) -> Optional[Dict[str, Any]]:
    """Nhận diện nhịp bẻ cầu sớm."""
    tail = labels[-12:] if len(labels) > 12 else labels[:]
    if len(tail) < 5:
        return None
    ...
    for side in (HIGH_LABEL, LOW_LABEL):
        ...
        if len(tail) >= 5 and tail[-5:-2] == [side, side, side] and tail[-2:] == [other, other]:
            return {"name": "BẺ CẦU SỚM", "detail": f"Nhịp {side} đang bẻ sang {other}", "score": 94, "hint": other}
        if len(tail) >= 4 and tail[-4:-1] == [side, side, side] and tail[-1] == other:
            return {"name": "BẺ CẦU SỚM", "detail": f"Vừa gãy khỏi nhịp {side} sang {other}", "score": 90, "hint": other}
        if len(tail) >= 6:
            recent = tail[-6:]
            changes = sum(1 for i in range(1, len(recent)) if recent[i] != recent[i - 1])
            if changes >= 4 and recent[-1] == other and recent[-2] == side:
                return {"name": "BẺ CẦU SỚM", "detail": f"Đuôi nhịp đổi gấp từ {side} sang {other}", "score": 86, "hint": other}
    return None


def detect_chaos_pattern(labels: List[str]) -> Optional[Dict[str, Any]]:
    tail = labels[-16:] if len(labels) > 16 else labels[:]
    if len(tail) < 8:
        return None
    changes = sum(1 for i in range(1, len(tail)) if tail[i] != tail[i - 1])
    changes_ratio = changes / max(1, len(tail) - 1)
    ent = entropy_score(tail, min(12, len(tail)))
    vol = volatility_score(tail, min(10, len(tail)))
    if changes_ratio >= 0.74 and ent >= 0.95 and vol >= 0.70:
        return {
            "name": "CẦU LOẠN",
            "detail": f"Đổi nhịp dày, entropy={ent:.2f}, vol={vol:.2f}",
            "score": 83,
            "hint": tail[-1] if tail else None,
        }
    return None

def detect_mixed_total_pattern(labels: List[str]) -> Optional[Dict[str, Any]]:
    runs = run_length_encode(labels)
    if len(runs) < 5:
        return None
    counts = [c for _, c in runs[-6:]]
    if len(counts) >= 5:
        # dạng tổng hợp: nhiều block độ dài khác nhau lặp/đổi liên tục
        uniq = len(set(counts))
        if uniq >= 3 and sum(1 for i in range(1, len(counts)) if counts[i] != counts[i - 1]) >= 3:
            return {
                "name": "CẦU TỔNG HỢP",
                "detail": f"Run-length hỗn hợp {'-'.join(map(str, counts[-5:]))}",
                "score": 85,
                "hint": labels[-1] if labels else None,
            }
    return None

def detect_lut_pattern(labels: List[str]) -> Optional[Dict[str, Any]]:
    tail = labels[-10:] if len(labels) > 10 else labels[:]
    if len(tail) < 6:
        return None
    # "cầu lộ" = pattern đã lộ rõ vì block ổn định và support cao
    block = run_length_encode(tail)
    if len(block) >= 3:
        lens = [c for _, c in block[-3:]]
        if min(lens) >= 2 and max(lens) <= 4:
            return {
                "name": "CẦU LỘ",
                "detail": f"Pattern lộ rõ với block { '-'.join(map(str, lens)) }",
                "score": 82,
                "hint": block[-1][0],
            }
    return None

def detect_bay_pattern(labels: List[str]) -> Optional[Dict[str, Any]]:
    # alias để bot hiểu cầu bẫy/cầu dụ là cùng họ
    trap = detect_false_trap(labels)
    if trap:
        trap = dict(trap)
        trap["name"] = "CẦU DỤ"
        return trap
...
def detect_all_patterns(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    if last in (LOW_LABEL, HIGH_LABEL) and streak >= 3:
        ...
        if streak >= 7:
            patterns.append({"name": "CẦU BẢY", "detail": f"Bệt rất dài {last} x{streak}", "score": min(90 + streak, 98), "hint": last})
...
def web_scan_history(labels: List[str], dice_rolls: Optional[List[List[int]]] = None) -> Optional[Dict[str, Any]]:
    ...
    for w in windows:
        ...
        for item in (
            ...
            *detect_dynamic_patterns(seg),
            ...
            detect_micro_break(seg),
            detect_chaos_pattern(seg),
            detect_mixed_total_pattern(seg),
            detect_lut_pattern(seg),
            detect_bay_pattern(seg),
    ...
    if best is None and dice_hint in (LOW_LABEL, HIGH_LABEL):
        best = {"name": "WEB QUÉT DICE", "detail": dice_note or f"Xí ngầu nghiêng {dice_hint}", "score": 72, "hint": dice_hint, "source": "web", "support": 0, "window": 0}
    return best
...
def deep_structure_analysis(labels: List[str]) -> Dict[str, Any]:
    ...
    chaos_raw = (change_rate * 35.0) + (ent * 20.0) + (vol * 20.0) + (irregularity * 25.0)
...
def prediction_gate(labels: List[str], report: Dict[str, Any], state: Optional[Dict[str, Any]] = None) -> Tuple[bool, str]:
    ...
    if state is not None:
        ...
        state["last_gate_status"] = "CHO PHÉP" if total >= MIN_PREDICTION_DATA else "TẠM DỪNG"
        state["last_gate_reason"] = reason
    if total < MIN_PREDICTION_DATA:
        return False, f"Chưa đủ {MIN_PREDICTION_DATA} dữ liệu"
    return True, "Đủ dữ liệu, quét và dự đoán luôn"
    ...
    if patterns:
        ...
        if hint in (LOW_LABEL, HIGH_LABEL):
            ...
            if name.startswith(("1-1", "XEN KẼ")):
                bonus += 2
            return {"label": hint, "confidence": clamp_confidence(int(primary.get("score", 60)) + bonus), "source": f"pattern:{name}"}
    if len(labels) < 4:
        return {"label": labels[-1] if labels else LOW_LABEL, "confidence": 60, "source": "pattern"}
    if labels[-1] == labels[-2] == labels[-3]:
        return {"label": labels[-1], "confidence": 66, "source": "pattern"}
    if labels[-1] != labels[-2]:
        return {"label": labels[-2], "confidence": 60, "source": "pattern"}
    return {"label": labels[-1], "confidence": 60, "source": "pattern"}

def predict_markov(labels: List[str]) -> Dict[str, Any]:
    if len(labels) < 3:
        return {"label": labels[-1] if labels else LOW_LABEL, "confidence": 60, "source": "markov"}
    ...
    if last in trans and trans[last]:
        ...
        return {"label": label, "confidence": clamp_confidence(confidence), "source": "markov"}
    if len(labels) >= 4:
        key = tuple(labels[-2:])
        ...
        if key in bigram_map and bigram_map[key]:
            ...
            return {"label": label, "confidence": clamp_confidence(confidence), "source": "markov"}
    return {"label": labels[-1], "confidence": 60, "source": "markov"}
...
def predict_structure(labels: List[str], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if patterns:
        ...
        if hint in (LOW_LABEL, HIGH_LABEL) and name in strong_same:
            ...
            return {"label": hint, "confidence": clamp_confidence(int(top.get("score", 60)) + bonus), "source": f"structure:{name}"}
        if name.startswith(("CẦU ", "LẶP MẪU", "LẶP KHỐI", "SÓNG", "CHU KỲ", "CHUYỂN PHA", "XEN KẼ")):
            return {"label": hint if hint in (LOW_LABEL, HIGH_LABEL) else (labels[-1] if labels else LOW_LABEL), "confidence": clamp_confidence(int(top.get("score", 60)) + 1), "source": f"structure:{name}"}
        if name == "CÂN BẰNG":
            return {"label": labels[-1] if labels else LOW_LABEL, "confidence": 60, "source": "structure:balance"}
    if len(labels) < 2:
        return {"label": labels[-1] if labels else LOW_LABEL, "confidence": 60, "source": "structure"}
    return {"label": labels[-1], "confidence": 60, "source": "structure"}

def predict_chart(labels: List[str], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    if not labels:
        return {"label": LOW_LABEL, "confidence": 60, "source": "chart"}
    ...
    confidence = clamp_confidence(base)
...
def predict_dice(dice_rolls: List[List[int]], summary: Dict[str, Any]) -> Dict[str, Any]:
    if not dice_rolls:
        return {"label": LOW_LABEL, "confidence": 60, "source": "dice"}
    ...
    return {"label": label, "confidence": clamp_confidence(confidence), "source": "dice"}

def predict_break_ai(labels: List[str], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    """Mô hình bẻ cầu sớm để bắt nhịp sắp gãy."""
    if not labels:
        return {"label": LOW_LABEL, "confidence": 60.0, "source": "break_ai"}

    tail = labels[-14:] if len(labels) > 14 else labels[:]
    last_label, streak = current_streak(tail)
    last_label = last_label or (labels[-1] if labels else LOW_LABEL)
    recent = tail[-6:] if len(tail) >= 6 else tail[:]
    prev = tail[-12:-6] if len(tail) >= 12 else tail[:-6]

    if not recent:
        return {"label": last_label, "confidence": 60.0, "source": "break_ai"}

    recent_changes = sum(1 for i in range(1, len(recent)) if recent[i] != recent[i - 1])
    prev_changes = sum(1 for i in range(1, len(prev)) if len(prev) > 1 and prev[i] != prev[i - 1]) if prev else 0
    recent_high = safe_div(recent.count(HIGH_LABEL), len(recent))
    prev_high = safe_div(prev.count(HIGH_LABEL), len(prev)) if prev else recent_high
    volatility_boost = float(report.get("volatility", 0.0)) * 18.0

    if last_label in (LOW_LABEL, HIGH_LABEL) and streak >= 3:
        if len(tail) >= 4 and tail[-1] != tail[-2]:
            label = tail[-1]
            confidence = 74.0 + min(4.0, streak * 0.5) + volatility_boost * 0.25
            return {"label": label, "confidence": clamp_confidence(confidence_jitter(confidence, spread=1.6)), "source": "break_ai:broke_now"}
        if recent_changes >= 2 and abs(recent_high - prev_high) >= 0.20:
            label = opposite_label(last_label)
            confidence = 70.0 + min(6.0, streak * 0.8) + volatility_boost * 0.20
            return {"label": label, "confidence": clamp_confidence(confidence_jitter(confidence, spread=1.6)), "source": "break_ai:flip_risk"}

    if recent_changes >= 4 and len(recent) >= 6:
        label = recent[-1]
        confidence = 68.0 + (recent_changes - 3) * 2.0 + volatility_boost * 0.20
        return {"label": label, "confidence": clamp_confidence(confidence_jitter(confidence, spread=1.4)), "source": "break_ai:shift"}

    if abs(recent_high - prev_high) >= 0.30:
        label = HIGH_LABEL if recent_high >= prev_high else LOW_LABEL
        confidence = 66.0 + abs(recent_high - prev_high) * 20.0
        return {"label": label, "confidence": clamp_confidence(confidence_jitter(confidence, spread=1.2)), "source": "break_ai:phase"}

    confidence = 60.0 + min(8.0, (recent_changes + prev_changes) * 1.5) + volatility_boost * 0.12
    return {"label": labels[-1], "confidence": clamp_confidence(confidence_jitter(confidence, spread=1.0)), "source": "break_ai:default"}
...
def filter_predictions_for_decision(
    ...
    thresholds = {"chart": 50, "dice": 50, "pattern": 50, "markov": 50, "structure": 50}

    for name, pred in predictions.items():
        label = pred.get("label")
        conf = float(pred.get("confidence", 50))
        keep = False
        if conf >= thresholds.get(name, 50):
            keep = True
        if not keep and label in (chart_label, structure_label, pattern_label):
            if float(chart.get("confidence", 0)) >= 58 or float(structure.get("confidence", 0)) >= 58 or int(top_pattern.get("score", 0)) >= 58:
                keep = True
        if keep:
            filtered[name] = pred
        else:
...
def meta_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for name, pred in filtered_predictions.items():
        ...
        strongest_conf = max(strongest_conf, int(conf))
    ...
    if not vote:
        ...
        conf = clamp_confidence(confidence_jitter(float(fallback.get("confidence", 50)), spread=1.4))
        return {
            "model": "fallback",
    ...
    recent_wr = recent_winrate(state, 20)
    overall_wr = overall_winrate(state)
    top_pattern = report.get("patterns", [{}])[0] if report.get("patterns") else {}
    pattern_score = int(top_pattern.get("score", 0))
    pattern_boost = max(0, pattern_score - 60) * 0.14
    history_boost = (recent_wr - 50) * 0.10 + (overall_wr - 50) * 0.05
    stability_boost = max(0.0, 70.0 - float(report.get("chaos_score", 0))) * 0.05

    raw_conf = (
        56.0
        + top_ratio * 18.0
        + agreement * 3.5
        + (strongest_conf - 50) * 0.22
        + pattern_boost
        + history_boost
        + stability_boost
    )
    if filtered_predictions.get("chart", {}).get("label") == best_label:
        raw_conf += 2.5
    if filtered_predictions.get("structure", {}).get("label") == best_label:
        raw_conf += 1.5
    if abs(trend) >= 0.18:
        raw_conf += 2.0
    if smoothness >= 0.65:
    ...
    # Làm confidence mềm hơn, có dao động nhỏ nhưng vẫn nằm trong band đã cấu hình
    confidence = clamp_confidence(confidence_jitter(raw_conf, spread=2.1))
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str], dice_rolls_full: Optional[List[List[int]]] = None) -> Dict[str, Any]:
    ...
    predictions = {
        ...
        "break_ai": predict_break_ai(labels, report, adv),
    }
    ...
    state["last_note"] = f"Model: {meta['model']} | Consensus: {meta.get('agreement', 0)}/{len(predictions)}"
    state["last_structure"] = report["structure"]
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Luồng hoạt động: nhập dữ liệu -> quét cầu -> dự đoán ngay -> robot chốt cuối."
    )
...
def build_trace_message(state: Dict[str, Any]) -> str:
    ...
    if isinstance(model_preds, dict) and model_preds:
        model_text = " | ".join(f"{k}:{v.get('label', '-')}"
                                f"({int(v.get('confidence', 0))}%)" for k, v in model_preds.items())
    else:
        model_text = "-"
    return (
