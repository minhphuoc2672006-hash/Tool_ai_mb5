...
MAX_FINAL_CONFIDENCE = int(os.getenv("MAX_FINAL_CONFIDENCE", "80"))
...
def clamp_confidence(value: float, low: float = float(MIN_FINAL_CONFIDENCE), high: float = float(MAX_FINAL_CONFIDENCE)) -> float:
    """Clamp confidence to a 60-80 band and keep one decimal."""
    try:
        v = float(value)
    except Exception:
        v = float(low)
...
def repair_state(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(d.get("model_accuracy"), dict):
        d["model_accuracy"] = {"pattern": 50, "markov": 50, "chart": 50, "structure": 50, "break_ai": 50}
    defaults = new_state()
...
def detect_micro_break(labels: List[str]) -> Optional[Dict[str, Any]]:
    """Nhận diện nhịp bẻ sớm: chuỗi đang bệt/đều nhưng vừa gãy 1-2 nhịp."""
    tail = labels[-12:] if len(labels) > 12 else labels[:]
    if len(tail) < 5:
        return None
    ...
    for side in (HIGH_LABEL, LOW_LABEL):
        ...
        # Bệt 3-4 nhịp rồi gãy ở cuối
        if len(tail) >= 5 and tail[-5:-2] == [side, side, side] and tail[-2:] == [other, other]:
            return {
                "name": "BẺ CẦU SỚM",
                "detail": f"Nhịp {side} đang bẻ sang {other}",
                "score": 94,
                "hint": other,
            }
        if len(tail) >= 4 and tail[-4:-1] == [side, side, side] and tail[-1] == other:
            return {
                "name": "BẺ CẦU SỚM",
                "detail": f"Vừa gãy khỏi nhịp {side} sang {other}",
                "score": 90,
                "hint": other,
            }

        # Đuôi đang xen kẽ nhưng vừa đổi nhịp mạnh
        if len(tail) >= 6:
            recent = tail[-6:]
            ...
            if changes >= 4 and recent[-1] == other and recent[-2] == side:
                return {
                    "name": "BẺ CẦU SỚM",
                    "detail": f"Đuôi nhịp đổi gấp từ {side} sang {other}",
                    "score": 86,
                    "hint": other,
                }
    return None
...
def predict_dice(dice_rolls: List[List[int]], summary: Dict[str, Any]) -> Dict[str, Any]:
    if not dice_rolls:
        return {"label": LOW_LABEL, "confidence": 60.0, "source": "dice"}
...
def predict_break_ai(labels: List[str], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    """Mô hình bẻ cầu sớm: ưu tiên nhận diện lúc nhịp vừa gãy/chuyển pha."""
    if not labels:
    ...
    # Khi bệt dài và vừa gãy 1-2 nhịp -> chọn phía mới
    if last_label in (LOW_LABEL, HIGH_LABEL) and streak >= 3:
        if len(tail) >= 4 and tail[-1] != tail[-2]:
            ...
            return {"label": label, "confidence": clamp_confidence(confidence), "source": "break_ai:broke_now"}
        if recent_changes >= 2 and abs(recent_high - prev_high) >= 0.20:
            ...
            return {"label": label, "confidence": clamp_confidence(confidence), "source": "break_ai:flip_risk"}

    # Đuôi đổi gấp thì đi theo đuôi mới
    if recent_changes >= 4 and len(recent) >= 6:
        label = recent[-1]
        confidence = 68.0 + (recent_changes - 3) * 2.0 + volatility_boost * 0.20
        return {"label": label, "confidence": clamp_confidence(confidence), "source": "break_ai:shift"}

    # Có tín hiệu đảo pha rõ
    if abs(recent_high - prev_high) >= 0.30:
        ...
        return {"label": label, "confidence": clamp_confidence(confidence), "source": "break_ai:phase"}

    # Mặc định bám theo nhịp gần nhất nhưng không quá mạnh
    confidence = 60.0 + min(8.0, (recent_changes + prev_changes) * 1.5) + volatility_boost * 0.12
    return {"label": labels[-1], "confidence": clamp_confidence(confidence), "source": "break_ai:default"}

def update_model_accuracy(state: Dict[str, Any], predictions: Dict[str, Dict[str, Any]], actual: str) -> None:
    state.setdefault("model_accuracy", {"pattern": 50, "markov": 50, "chart": 50, "structure": 50, "break_ai": 50})
...
def meta_decision(predictions: Dict[str, Dict[str, Any]], state: Dict[str, Any], report: Dict[str, Any], adv: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not vote:
        ...
        conf = clamp_confidence(float(fallback.get("confidence", 50)))
        return {
            "model": "fallback",
...
def analyze_state_from_labels(state: Dict[str, Any], labels_full: List[str], dice_rolls_full: Optional[List[List[int]]] = None) -> Dict[str, Any]:
    ...
    break_ai = predict_break_ai(labels, report, adv)
    state["last_break_name"] = str(break_ai.get("source", "break_ai"))
    state["last_break_detail"] = f"{break_ai.get('label', '-')}: {break_ai.get('source', 'break_ai')}"
    state["last_break_score"] = float(break_ai.get("confidence", 0))
...
def build_final_message(meta: Dict[str, Any], state: Dict[str, Any]) -> str:
    return (
        ...
        f"📊 Tỷ lệ: {float(meta.get('confidence', 0)):.1f}%\n"
        ...
        f"🪓 Break AI: {state.get('last_break_detail') or '-'} ({float(state.get('last_break_score', 0)):.1f}%)\n"
...
def build_trace_message(state: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"• Confidence: {float(state.get('last_prediction_conf', 0)):.1f}%\n"
        f"• Win / Lose: {state.get('prediction_hits', 0)} / {state.get('prediction_misses', 0)}\n"
