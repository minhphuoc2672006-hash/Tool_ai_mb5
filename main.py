...
ADMIN_USER_ID = int(os.getenv("ADMIN_USER_ID", "0"))  # điền Telegram user ID của bạn vào biến môi trường
...
def detect_reversal(labels: List[str]) -> Optional[Dict[str, Any]]:
    """
    Phát hiện nhịp đảo chiều gần đây.
    Chỉ trả về khi có đủ dữ liệu và tín hiệu đổi pha tương đối rõ.
    """
    ...
    if diff >= 0.50:
        ...
        return {
            "name": "ĐẢO CHIỀU",
            "detail": f"6 mẫu gần nhất nghiêng mạnh về {hint}",
            "score": 86,
            "hint": hint,
        }
    ...
    if diff >= 0.30 and abs(recent_vol - prev_vol) >= 0.20:
        ...
        return {
            "name": "ĐẢO CHIỀU",
            "detail": f"Nhịp gần đây đổi pha về {hint}",
            "score": 80,
            "hint": hint,
        }
