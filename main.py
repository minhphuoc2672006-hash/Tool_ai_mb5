...
def generate_strategies():
    ...
    # Tạo thêm biến thể random để thành 5000 chiến thuật
    strategies = base.copy()
    while len(strategies)<5000:
        s = random.choice(base)
        # Thêm biến thể
        variant = f"{s}_{random.randint(1,9999)}"
...
def run_strategy(name, history):
    tx = get_tx_history(history)
    if not tx: return random.choice(["Tài","Xỉu"])
    # Tách tên cơ bản nếu là biến thể
    base_name = name.split("_")[0]
    if base_name == "FOLLOW": return tx[-1]
    if base_name == "OPPOSITE": return "Xỉu" if tx[-1]=="Tài" else "Tài"
    if base_name == "TREND":
        if len(tx)>=4: return "Tài" if tx[-4:].count("Tài") > tx[-4:].count("Xỉu") else "Xỉu"
    if base_name == "STREAK":
        if len(tx)>=2 and tx[-1]==tx[-2]: return tx[-1]
    if base_name == "ANTI_STREAK":
        if len(tx)>=2 and tx[-1]==tx[-2]: return "Xỉu" if tx[-1]=="Tài" else "Tài"
    if base_name == "WEIGHTED":
        c = Counter(tx)
    ...
    if base_name == "CHAOS": return random.choice(tx)
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user["last_pred"] is not None:
        if user["last_pred"]==real:
            ...
            # 🔥 PHÂN TÍCH TOÀN BỘ 5000 CHIẾN THUẬT
            strategy_scores = {s:0 for s in STRATEGIES}
            tx_history = get_tx_history(user["history"])
            for i in range(1,len(tx_history)):
                real_i = tx_history[i]
                for s in STRATEGIES:
                    pred = run_strategy(s, user["history"][:i])
                    if pred == real_i:
                        strategy_scores[s] += 1
            best_strategy = max(strategy_scores.items(), key=lambda x:x[1])[0]
            old_strategy = user["strategy"]
            user["strategy"] = best_strategy
            logging.info(f"⚡ Thua, đổi chiến thuật: {old_strategy} → {user['strategy']}")
...
def main():
    ...
    print("🔥 AI CASINO PRO MAX 7.0 RUNNING...")
    app.run_polling()
